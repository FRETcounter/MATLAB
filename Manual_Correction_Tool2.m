...
function I = Manual_Correction_Tool2 (I, ground_truth, Memb_Array, FileName_ground_truth, Invert_Raw_Image, pathName)
...
while again && lineCount < 10000
    ...
    try
        if choice ==1  % To move to the keyboard mode
            burnedImage = Membrane_Correction_Magnification_Mode(cumulativeBinaryImage, ground_truth, burnedImage, ground_truth, Fuse_I);
            BW =  ~burnedImage;
        end
        ...
        if choice ==7   %Save Work
            ...
            Reference_Mask = burnedImage;      % reference mask
            imwrite(Reference_Mask, [Subfolder_path_and_name,['\Mask_', FileName_ground_truth]], 'tif')
        end
        
        %**********************************************************************
        %**********************************************************************
        
        if choice ==8  %Overlap Old Result
        ...
        if choice ==14  %Free Hand Draw (NEW one never fail)
            f.GraphicsSmoothing = 'off';
            opengl software
